...
// Create a mock client if credentials are missing or Supabase is disabled
const createSupabaseClient = (): SupabaseClient => {
  if (!supabaseEnabled || !supabaseUrl || !supabaseAnonKey) {
    if (!supabaseEnabled) {
      console.info('Supabase disabled via NEXT_PUBLIC_SUPABASE_ENABLED=false');
    } else {
      console.warn('Supabase credentials not configured');
    }
    // Return a client that won't attempt connections
