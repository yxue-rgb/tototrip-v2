...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      try {
        ...
        if (error) {
          console.warn('Supabase connection failed, running in guest mode:', error.message);
          setLoading(false);
          return;
        }
        ...
        console.warn('Supabase initialization failed, running in guest mode:', error);
        setLoading(false);
      }
    };
  ...
  const signup = async (data: SignupData) => {
    if (!isSupabaseEnabled) {
      return {
        success: false,
        error: 'Authentication is not configured yet. Please check back later or contact support.',
      };
    }
